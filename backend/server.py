...
# ============== Strategy Routes (Supabase with MongoDB fallback) ==============

def _use_supabase_strategies():
    """Check if we should use Supabase for strategies (when MongoDB is not available)"""
    return strategies_col is None and is_supabase_configured()

@api_router.get("/strategies")
async def list_strategies(request: Request):
    """List all strategies - any authenticated user can read"""
    await get_current_user_from_request(request)
    if _use_supabase_strategies():
        try:
            sb = get_supabase_admin()
            result = sb.table('strategies').select('*').execute()
            return result.data or []
        except Exception as e:
            logger.warning(f"Supabase strategies error: {e}")
            return []

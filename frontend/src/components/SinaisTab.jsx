...
const NUMBER_INFO = {
  ...
  1: { parity: 'ÃMPAR', highLow: 'BAIXO', refs: '2/6' },
  2: { parity: 'PAR', highLow: 'BAIXO', refs: '1/3/7' },
  3: { parity: 'ÃMPAR', highLow: 'BAIXO', refs: '2/4/8' },
  4: { parity: 'PAR', highLow: 'BAIXO', refs: '1/3/8' },
  5: { parity: 'ÃMPAR', highLow: 'BAIXO', refs: '1/2/6' },
  6: { parity: 'PAR', highLow: 'BAIXO', refs: '1/5/7/9' },
  7: { parity: 'ÃMPAR', highLow: 'BAIXO', refs: '1/2/6/7' },
  8: { parity: 'PAR', highLow: 'BAIXO', refs: '1/3/5' },
  9: { parity: 'ÃMPAR', highLow: 'BAIXO', refs: '4/2' },
  10: { parity: 'PAR', highLow: 'BAIXO', refs: '5/1' },
  11: { parity: 'ÃMPAR', highLow: 'BAIXO', refs: '3/9' },
  12: { parity: 'PAR', highLow: 'BAIXO', refs: '1/2/6/8' },
  13: { parity: 'ÃMPAR', highLow: 'BAIXO', refs: '3/5/9' },
  14: { parity: 'PAR', highLow: 'BAIXO', refs: '4/2' },
  15: { parity: 'ÃMPAR', highLow: 'BAIXO', refs: '1/5/8' },
  16: { parity: 'PAR', highLow: 'BAIXO', refs: '6/2' },
  17: { parity: 'ÃMPAR', highLow: 'BAIXO', refs: '1/3/7' },
  18: { parity: 'PAR', highLow: 'BAIXO', refs: '2/4/7' },
  19: { parity: 'ÃMPAR', highLow: 'ALTO', refs: '4/6' },
  20: { parity: 'PAR', highLow: 'ALTO', refs: '1/3/5' },
  21: { parity: 'ÃMPAR', highLow: 'ALTO', refs: '2/4' },
  22: { parity: 'PAR', highLow: 'ALTO', refs: '7/9' },
  23: { parity: 'ÃMPAR', highLow: 'ALTO', refs: '1/8' },
  24: { parity: 'PAR', highLow: 'ALTO', refs: '5/7' },
  25: { parity: 'ÃMPAR', highLow: 'ALTO', refs: '2/6/8' },
  26: { parity: 'PAR', highLow: 'ALTO', refs: '3/0' },
  27: { parity: 'ÃMPAR', highLow: 'ALTO', refs: '2/4/6' },
  28: { parity: 'PAR', highLow: 'ALTO', refs: '1/3/7' },
  29: { parity: 'ÃMPAR', highLow: 'ALTO', refs: '7/9' },
  30: { parity: 'PAR', highLow: 'ALTO', refs: '2/8' },
  31: { parity: 'ÃMPAR', highLow: 'ALTO', refs: '3/5/9' },
  32: { parity: 'PAR', highLow: 'ALTO', refs: '4/6/0' },
  33: { parity: 'ÃMPAR', highLow: 'ALTO', refs: '1/5/7' },
  34: { parity: 'PAR', highLow: 'ALTO', refs: '6/8' },
  35: { parity: 'ÃMPAR', highLow: 'ALTO', refs: '1/3' },
...
const detectFBPattern = (a, b, c) => {
  ...
  // b and c share root, a is different â†’ target = a (oldest)
  if (db === dc && da !== dc) return { formed: [c, b, a], remaining: a, entry: getFBEntry(a) };
  // a and c share root, b is different â†’ target = b (middle)
  if (da === dc && db !== dc) return { formed: [c, b, a], remaining: b, entry: getFBEntry(b) };
  // a,b match â†’ target would be c (newest) â†’ INVALID, do not form
  return null;
};
...
const RadarTab = ({ viewMode = "vertical" }) => {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      try {
        ...
        if (data.source === 'sinais' && data.time > lastTime) {
          lastTime = data.time;
          setGiros(prev => {
            const newGiros = [...prev, data.number];
            if (newGiros.length > limiteGiros) return newGiros.slice(-limiteGiros);
            return newGiros;
          });
          setAddCount(c => c + 1);
        }
  ...
  const addNumber = (n) => {
    ...
    setAddCount(c => c + 1);
    // Sync: notify other tabs about new number
    localStorage.setItem('sync_new_number', JSON.stringify({ number: n, time: Date.now(), source: 'radar' }));
  };
        ...
        {hasData ? (
          <>
            <span className={`text-white font-bold ${compact ? "text-sm" : ""}`}>
              FAMÃLIA: <span data-testid="family-target">{alvoFinal}</span>
            </span>
          ...
          <span className="text-gray-600 text-xs">Clique nos nÃºmeros para ver a famÃ­lia</span>
        )}
      </div>
    );
  };
  ...
  const JuncaoCard = ({ compact }) => {
    ...
    return (
      <div className={`card-glass ${compact ? "!p-2 h-full flex flex-col" : ""}`} data-testid="juncao-card">
        <span className="label-accent" style={{ color: '#fff', borderColor: '#D4AF37', fontSize: compact ? '0.7rem' : '0.9rem' }}>JUNÃ‡ÃƒO DOS NÃšMEROS</span>
        <div className="flex-1 overflow-y-auto" style={{ scrollbarWidth: 'auto', scrollbarColor: '#D4AF37 #222' }}>
          {repeatedNums.length > 0 ? repeatedNums.map((num) => {
            ...
            <div className={`text-center text-gray-600 ${compact ? "py-2 text-xs" : "py-4 text-sm"}`}>
              NÃºmeros repetidos aparecerÃ£o aqui
            </div>
          )}
        </div>
      </div>
  ...
  const EstrategiaFBCard = ({ compact }) => {
    return (
      <div className={`card-glass border-2 border-[#D4AF37] ${compact ? "!p-2 h-full flex flex-col" : ""}`} data-testid="fb-card">
        <span className="label-accent" style={{ color: '#fff', borderColor: '#D4AF37', fontSize: compact ? '0.7rem' : '0.9rem' }}>ESTRATÃ‰GIA FB</span>
